...
// get MUNTHRDS as the argument1 and VECLEN as argument2
if ( argc != 3 ) {
  printf("usage: %s 10 100000\n", argv[0]);
  exit(0);
}
// if argument is 0 or parses to 0, set NUMTHRDS to 1 by default
int NUMTHRDS = strtol(argv[1], (char **)NULL, 10) > 0 ? strtol(argv[1], (char **)NULL, 10) : 1;

// if non-number string is passed as argument2, sum is 0.
int VECLEN = strtol(argv[2], (char **)NULL, 10);
pthread_t callThd[NUMTHRDS];
